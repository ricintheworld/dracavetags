...
public final class VaultScreen implements ClickableScreen {
    ...
    public void open() {
        ...
        for (int i = from; i < to; i++) {
            DCTag t = tags.get(i); int idx = i - from;
            if (idx >= cslots.size()) break;
            int s = cslots.get(idx);
        ...
        switch (actions.getOrDefault(rawSlot, IconAction.NONE)) {
            ...
            default -> {
                ...
                if (tid != null) {
                    ...
                    close();
                    CompletableFuture<DCTagResult> op = d != null && tid.equals(d.equippedId())
                            ? plugin.tagEngine().clear(viewer.getUniqueId())
                            : plugin.tagEngine().equip(viewer.getUniqueId(), tid);
                    op.thenAccept(result -> SchedulerUtil.runTask(plugin, () -> {
                        if (!viewer.isOnline()) return;
                        if (result == DCTagResult.SUCCESS || result == DCTagResult.COOLDOWN) {
                            plugin.screenSound().success(viewer);
                        } else {
                            plugin.screenSound().error(viewer);
                        }
                        new VaultScreen(plugin, viewer, page).open();
                    }));
                }
            }
        }
            ...
            if (headSlot >= 0) {
                ...
                if (hi != null) {
                    ...
                    if (hm != null) {
                        ...
                        String eqId = plugin.tagEngine().getCached(viewer.getUniqueId()) != null
                                ? plugin.tagEngine().getCached(viewer.getUniqueId()).equippedId() : null;
                        var eqTag = eqId != null ? plugin.registry().get(eqId) : null;
