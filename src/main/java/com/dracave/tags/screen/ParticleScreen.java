...
public final class ParticleScreen implements ClickableScreen {
    ...
    private static final String[][] PRESETS = {
            ...
            {"DRIPPING_LAVA", "滴落岩浆", "DRIPSTONE_BLOCK"},
            {"DRIPPING_WATER", "滴落水", "DRIPSTONE_BLOCK"},
            {"DRIPPING_HONEY", "滴落蜂蜜", "HONEY_BLOCK"},
            {"DRIPPING_OBSIDIAN_TEAR", "滴落黑曜石泪", "CRYING_OBSIDIAN"},
            {"FALLING_HONEY", "飘落蜂蜜", "HONEY_BLOCK"},
            {"FALLING_OBSIDIAN_TEAR", "飘落黑曜石泪", "CRYING_OBSIDIAN"},
            {"LANDING_LAVA", "落地岩浆", "MAGMA_BLOCK"},
            {"LANDING_HONEY", "落地蜂蜜", "HONEY_BLOCK"},
            {"SHRIEK", "尖啸", "SCULK_SHRIEKER"},
            {"TINTED_LEAVES", "遮光树叶", "TINTED_GLASS"},
            {"TOWN_AURA", "村庄光环", "EMERALD"},
            {"WHITE_SMOKE", "白烟", "WHITE_WOOL"},
            {"DUST_PLUME", "红石羽流", "REDSTONE"},
            {"DUST_PILLAR", "尘埃柱", "REDSTONE"},
            {"BLOCK_CRUMBLE", "方块崩塌", "STONE"},
            {"BLOCK_MARKER", "方块标记", "STRUCTURE_BLOCK"},
            {"COPPER_FIRE_FLAME", "铜焰", "COPPER_BLOCK"},
            {"MOB_APPEARANCE", "怪物出现", "SPAWNER"},
            {"BARRIER", "屏障", "BARRIER"},
            {"VAULT", "宝库", "VAULT"},
            {"PALE_OAK_LEAVES", "苍白橡叶", "PALE_OAK_LEAVES"},
    };

    private final DraCaveTags plugin;
    private final Player player;
    private final Consumer<DCTagPart> onPick;
    private final Runnable onCancel;
    private final List<String[]> entries = new ArrayList<>();
    private int page;
    private Inventory inventory;

    public ParticleScreen(DraCaveTags plugin, Player player, Consumer<DCTagPart> onPick, Runnable onCancel) {
        this.plugin = plugin;
        this.player = player;
        this.onPick = onPick;
        this.onCancel = onCancel;
        for (String[] preset : PRESETS) {
            if (ParticleEngine.validParticle(preset[0])) {
                entries.add(preset);
            }
        }
    }
    ...
    public void click(int rawSlot, ClickType clickType) {
        if (rawSlot == SLOT_BACK) {
            ...
            switch (type) {
                ...
                default -> onPick.accept(new DCTagPart(type, null, List.of()));
            }
        }
    }
    ...
    private void promptManual() {
        ...
        plugin.chatPrompt().prompt(player, "§e请输入粒子（如 DUST #FF0000，最多 3 个颜色）：", (p, value) -> {
            String[] parts = value.trim().split("\\s+");
            String type = parts[0].toUpperCase();
            if (!ParticleEngine.validParticle(type)) {
            ...
            onPick.accept(new DCTagPart(type, null, colors));
        }, true);
    }

    private void pickColors(String type, int needed, List<String> picked) {
        if (picked.size() >= needed) {
            onPick.accept(new DCTagPart(type, null, List.copyOf(picked)));
            return;
        }
