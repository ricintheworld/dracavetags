...
public final class ShopScreen implements ClickableScreen {
    ...
    public void open() {
        ...
        List<DCTag> tags = plugin.registry().all().stream()
                .filter(t -> t.purchaseOffer() != null && !t.shopHidden() && (filter == null || t.purchaseOffer().currency() == filter))
        ...
        for (int i = from; i < Math.min(tags.size(), from + PAGE_SIZE); i++) {
            int s = i - from; slots.put(s, tags.get(i)); inventory.setItem(s, tagItem(tags.get(i)));
        }
